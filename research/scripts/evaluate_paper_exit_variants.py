...
def main() -> None:
    output_dir = Path("research/results/exit_variant_experiments")
    plot_dir = Path("research/results/plots")
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_dir.mkdir(parents=True, exist_ok=True)

    pool_config = V4_POOLS["uni-base"]
    events = load_v4_events("research/data/uni_base_pool_history.csv", pool_id=pool_config.pool_address)
    spec = WindowSpec(
        mode="swap_count",
        train_swaps=200,
        val_swaps=50,
