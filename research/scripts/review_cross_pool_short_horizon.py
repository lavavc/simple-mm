"""Atomically stamp a reviewed short-horizon evidence package."""
# ruff: noqa: E402
...
class ReviewCliError(RuntimeError):
    """Raised for review argument errors before any mutation."""


class _FailClosedArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> Never:
        raise ReviewCliError(message)


def build_parser() -> argparse.ArgumentParser:
    parser = _FailClosedArgumentParser(
        description="Review and stamp a QA-pass short-horizon evidence package",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"short-horizon review failed: {message}", file=sys.stderr)
        return 1
