...
Historical companion to `research/autoresearch/lp.md`. Each entry follows the LP
...
**Method.** Queried public BSC RPC and Alchemy Base RPC for `eth_getTransactionReceipt` on the 12 BSC + 80 Base liquidity-event tx hashes in `research/data/uni_bsc_pool_history.csv` and `research/data/uni_base_pool_history.csv`. Computed `gas_used × effectiveGasPrice × native_usd / 1e18`. Used Coinbase spot at the time of analysis: BNB = $652.16, ETH = $2,267.92.

**Saved:** `research/results/h7/h7_base_gas_costs.csv`.
...
**Saved outputs:**
- `research/results/h7/uni_bsc_paper_full_grid_calibrated.csv`
- `research/results/h7/uni_bsc_{paper,ewma}_swapwf_calibrated_{aggregate,windows,summary}.csv`
...
**Implementation:**
- `research/backtester/metrics.py`: extended `composite_objective(net_return, max_dd, fees=0, tx_cost=0, fee_cost_alpha=0.001)`. Added `fee_cost_log_ratio` helper clamped to [-3, +2].
- `research/backtester/run.py`: `_compute_metrics` passes `sim.total_fees`, `sim.total_transaction_cost`. Aggregator emits `mean_validation_fee_to_tx_cost_ratio` column. Eligibility filter adds `mean_fee_cost_ratio >= min_fee_cost_ratio` (default 1.0).
- `research/tests/test_backtester.py`: 9 new tests (composite bonus/penalty/clamping, eligibility gate). All 72 tests pass.
...
**Saved outputs:** `research/results/h7_h6/uni_{bsc,base}_{paper,ewma}_swapwf_{aggregate,windows,summary}.csv`.
...
**Saved outputs:** `research/results/h7_h6_topn100/uni_{bsc,base}_{paper,ewma}_swapwf_*.csv`.
...
**Command:** `python research/scripts/h4_base_paper_fine_grid.py` (calibrated gas, top_n=100, swap-count WF 200/50/50)

**Saved outputs:** `research/results/h4/uni_base_paper_fine_grid_{aggregate,windows}.csv`.
...
5. **PBO tooling.** `--matrix-output` writes the full configs × windows
   validation matrix; `research/scripts/compute_pbo.py` computes CSCV PBO
...
**Tooling (committed).** `research/backtester/sizing.py` (SizingPolicy protocol with
...
simulator, `research/scripts/h12_capital_sweep.py` (capacity-curve artifact),
`research/scripts/h12_capacity_analysis.py` (paired marginals, share collapse,
analytic C*).
...
- **Run H12 on the extended dataset** (after the re-run confirms or revises
  the frozen winner set in `research/scripts/h12_capital_sweep.py`).
