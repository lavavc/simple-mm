...
### Backtester Sync (`research/backtester/strategy.py`) ← Phase 1 complete
...
**`research/scripts/fetch_cngn_balance_history.py`**
...
**`research/scripts/plot_fair_prices.py`**
...
```
python research/scripts/plot_fair_prices.py \
    --csv "research/data/Aerodrome&Pancakeswap_HistoricalData.csv" \
    --format legacy \
    --balance-csv research/data/cngn_balance_history.csv \
    --out fair_prices.png
```
...
#### 2a. `FairPriceScenario` dataclass (`research/backtester/params.py`)
...
#### 2b. Inventory injection hook (`research/backtester/simulator.py`)
...
already implemented in `research/scripts/plot_fair_prices.py` — move to `research/backtester/data.py`).

#### 2c. `--fair-price-grid` mode (`research/backtester/run.py`)

```
python -m research.backtester.run \
    --csv research/data/uni_base_v4.csv \
    --pool uni-base \
    --dataset-format v4 \
    --walkforward \
    --balance-csv research/data/cngn_balance_history.csv \
...
#### 2e. Tests (`research/tests/test_backtester.py`)
...
not transfer to production. Now that `research/backtester/strategy.py` is synced, that session
