...
Current implementation status: `research/backtester/v4_lp_ledger.py` defines the pure
ledger row builder, ownership matching logic, receipt ownership decoder, and
PositionManager `modifyLiquidities` action decoder. `research/scripts/export_v4_lp_ledger.py`
...
`research/backtester/lp_paper_episodes.py` now defines the first pure episode
...
Current implementation status: `research/scripts/export_tx_receipts.py` exports the
...
`research/backtester/lp_episode_features.py` and
`research/scripts/export_lp_episode_features.py` now provide the first gas-aware
...
`research/scripts/build_pool_feature_table.py` now builds causal swap-level pool
feature tables with lifetime cone percentiles and optional finite-lookback cone
columns through `--cone-lookback-seconds`. Existing lifetime columns are
preserved for compatibility; lookback columns are appended with suffixes such as
`_1h`, `_1d`, and `_7d`.
...
`research/scripts/report_pool_feature_stress_slices.py` now reports UTC calendar
stress slices from any selected cone fields. It keeps missing values explicit so
regime tests can distinguish "not stressed" from "feature unavailable".
