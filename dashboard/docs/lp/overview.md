...
Range width and rerange triggers are calculated from each venue's own pool
history. `uni-base` uses `uni-base_pool` snapshots and `uni-bsc` uses
`uni-bsc_pool` snapshots. When the scheduler can compute a market-layer
`StrategyFairPrice`, the LP strategy may use it as the range center; otherwise
it falls back to the venue-local EWMA center. This optional center override does
not use arb route state.
